...
function centers(count: number): number[] {
  const slot = TOTAL_H / count;
  return Array.from({ length: count }, (_, i) => i * slot + slot / 2);
}

const r32Y = centers(16);
const r16Y = centers(8);
const qfY = centers(4);
const sfY = centers(2);
const finalY = centers(1);
...
// ── R32 bracket data (top-to-bottom order follows bracket tree) ──
// SF M101 half
const r32Top: MatchData[] = [
...
const allR32 = [...r32Top, ...r32Bottom];

// R16 (8 matches, paired from R32)
const r16Data: { num: number; homeRef: string; awayRef: string }[] = [
...
// QF (4 matches)
const qfData: { num: number; homeRef: string; awayRef: string }[] = [
...
// SF (2 matches)
const sfData: { num: number; homeRef: string; awayRef: string }[] = [
  { num: 101, homeRef: "M97", awayRef: "M98" },
  { num: 102, homeRef: "M99", awayRef: "M100" },
];

// ── Connector component ──────────────────────────────────
function BracketConnectors({ inputCenters, outputCenters }: { inputCenters: number[]; outputCenters: number[] }) {
  ...
  for (let i = 0; i < outputCenters.length; i++) {
    ...
    // Horizontal from vertical midpoint to right edge
    lines.push(<div key={`ho-${i}`} className="absolute border-t-2 border-gray-300" style={{ top: yOut, left: midX, width: CONN_W - midX }} />);
  }

  return (
    <div className="relative shrink-0" style={{ width: CONN_W, height: TOTAL_H }}>
      {lines}
    </div>
  );
}
...
function TeamSlot({
  ...
  return (
    <div
      className={`flex items-center gap-1 px-2 ${borderClass}`}
      style={{ height: TEAM_H }}
    >
      {flags && (
...
function MatchBox({
  ...
  match: MatchData | { num: number; homeRef: string; awayRef: string };
  groupFlags: Record<string, string>;
  locale: string;
  isR32: boolean;
  width: number;
}) {
  if (isR32 && "home" in match) {
    return (
      <div
        className="bg-white border border-gray-300 rounded shadow-sm overflow-hidden"
        style={{ width, height: MATCH_H }}
      >
        <TeamSlot slot={match.home} groupFlags={groupFlags} locale={locale} position="home" />
        <TeamSlot slot={match.away} groupFlags={groupFlags} locale={locale} position="away" />
      </div>
    );
  }

  // Knockout match (R16+)
  const m = match as { num: number; homeRef: string; awayRef: string };
  return (
    <div
      className="bg-white border border-gray-300 rounded shadow-sm overflow-hidden"
      style={{ width, height: MATCH_H }}
    >
      <div
        className="flex items-center px-2 border-b border-gray-200 text-xs font-medium text-gray-600"
        style={{ height: TEAM_H }}
      >
        <span>{m.homeRef}{locale === "ja" ? "勝者" : " W"}</span>
      </div>
      <div
        className="flex items-center px-2 text-xs font-medium text-gray-600"
        style={{ height: TEAM_H }}
      >
        <span>{m.awayRef}{locale === "ja" ? "勝者" : " W"}</span>
      </div>
    </div>
  );
}
...
function RoundColumn({
  ...
  matches: (MatchData | { num: number; homeRef: string; awayRef: string })[];
  yCenters: number[];
  groupFlags: Record<string, string>;
  locale: string;
  isR32: boolean;
  ...
  return (
    <div className="relative shrink-0" style={{ width, height: TOTAL_H }}>
      {/* Round header */}
      <div
        className="absolute -top-8 left-0 right-0 text-center text-[11px] font-bold text-gray-500 uppercase tracking-wider"
      >
        {roundLabel}
      </div>
      {matches.map((match, i) => (
        <div
          key={match.num}
          className="absolute"
          style={{ top: yCenters[i] - MATCH_H / 2, left: 0 }}
        >
          <div className="absolute -top-3.5 left-1 text-[9px] font-semibold text-[#E8192C]">
            M{match.num}
          </div>
          <MatchBox match={match} groupFlags={groupFlags} locale={locale} isR32={isR32} width={width} />
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-wrap gap-3 mt-4">
          <div className="flex items-center gap-1.5 text-xs text-gray-600">
            ...
            {locale === "ja" ? "1位・2位（国旗付き）" : "1st/2nd place (with flags)"}
          </div>
          <div className="flex items-center gap-1.5 text-xs text-gray-600">
            <span className="inline-block w-3 h-3 rounded bg-gray-50 border border-dashed border-gray-300" />
            {locale === "ja" ? "3位（グループ名のみ）" : "3rd place (group letters only)"}
          </div>
        </div>
      </div>
      ...
      <div className="overflow-x-auto pb-4">
        ...
        >
          <div className="absolute top-8 left-0 flex items-start" style={{ height: TOTAL_H }}>
            {/* R32 */}
            <RoundColumn
              matches={allR32}
              yCenters={r32Y}
              ...
              isR32
              width={R32_COL_W}
              roundLabel={locale === "ja" ? "ラウンド32" : "Round of 32"}
            />

            {/* R32 → R16 connector */}
            <BracketConnectors inputCenters={r32Y} outputCenters={r16Y} />
            ...
            <RoundColumn
              ...
              isR32={false}
              width={COL_W}
              roundLabel={locale === "ja" ? "ラウンド16" : "Round of 16"}
            />

            {/* R16 → QF connector */}
            <BracketConnectors inputCenters={r16Y} outputCenters={qfY} />
            ...
            <RoundColumn
              ...
              isR32={false}
              width={COL_W}
              roundLabel={locale === "ja" ? "準々決勝" : "Quarter-finals"}
            />

            {/* QF → SF connector */}
            <BracketConnectors inputCenters={qfY} outputCenters={sfY} />
            ...
            <RoundColumn
              ...
              isR32={false}
              width={COL_W}
              roundLabel={locale === "ja" ? "準決勝" : "Semi-finals"}
            />

            {/* SF → Final connector */}
            <BracketConnectors inputCenters={sfY} outputCenters={finalY} />

            {/* Final */}
            <div className="relative shrink-0" style={{ width: FINAL_W, height: TOTAL_H }}>
              <div className="absolute -top-8 left-0 right-0 text-center text-[11px] font-bold text-amber-600 uppercase tracking-wider">
                {locale === "ja" ? "決勝" : "Final"}
              </div>
              <div
                className="absolute"
                style={{ top: finalY[0] - MATCH_H / 2, left: 0 }}
              >
                <div className="absolute -top-3.5 left-1 text-[9px] font-semibold text-amber-600">
                  M104
                </div>
                <div
                ...
                >
                  <div
                    className="flex items-center px-2 border-b border-amber-200 text-xs font-bold text-amber-800"
                    style={{ height: TEAM_H }}
                  >
                    M101{locale === "ja" ? "勝者" : " Winner"}
                  </div>
                  <div
                    className="flex items-center px-2 text-xs font-bold text-amber-800"
                    style={{ height: TEAM_H }}
                  >
              ...
              <div
                className="absolute"
                style={{ top: finalY[0] + MATCH_H / 2 + 32, left: 0 }}
              >
                <div className="absolute -top-3.5 left-1 text-[9px] font-semibold text-green-700">
                  M103
                </div>
                <div
                ...
                >
                  <div
                    className="flex items-center px-2 border-b border-green-200 text-xs font-medium text-green-700"
                    style={{ height: TEAM_H }}
                  >
                    M101{locale === "ja" ? "敗者" : " Loser"}
                  </div>
                  <div
                    className="flex items-center px-2 text-xs font-medium text-green-700"
                    style={{ height: TEAM_H }}
                  >
                    M102{locale === "ja" ? "敗者" : " Loser"}
                  </div>
                </div>
                <div className="text-[10px] text-green-600 font-medium mt-1 text-center">
          ...
          {/* Dashed line separator between upper/lower halves */}
          <div
          ...
          {/* Half labels */}
          <div
            className="absolute text-[10px] font-bold text-blue-400 bg-white px-1"
            style={{ top: 32 + TOTAL_H / 2 - 20, left: R32_COL_W + CONN_W }}
          >
            {locale === "ja" ? "SF M101" : "SF M101"}
          </div>
          <div
            className="absolute text-[10px] font-bold text-purple-400 bg-white px-1"
            style={{ top: 32 + TOTAL_H / 2 + 8, left: R32_COL_W + CONN_W }}
          >
            {locale === "ja" ? "SF M102" : "SF M102"}
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h3 className="text-sm font-bold text-blue-800 mb-2">
            {locale === "ja" ? "上半分 (準決勝 M101)" : "Upper Half (Semi-final M101)"}
          </h3>
          <p className="text-xs text-blue-600">
            {locale === "ja"
              ? "M73, M76, M75, M78, M83, M84, M81, M82 → M89, M90, M93, M94 → M97, M98 → M101"
              : "M73, M76, M75, M78, M83, M84, M81, M82 → M89, M90, M93, M94 → M97, M98 → M101"}
          </p>
        </div>
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
          <h3 className="text-sm font-bold text-purple-800 mb-2">
            {locale === "ja" ? "下半分 (準決勝 M102)" : "Lower Half (Semi-final M102)"}
          </h3>
          <p className="text-xs text-purple-600">
            {locale === "ja"
              ? "M74, M77, M79, M80, M87, M86, M85, M88 → M91, M92, M95, M96 → M99, M100 → M102"
              : "M74, M77, M79, M80, M87, M86, M85, M88 → M91, M92, M95, M96 → M99, M100 → M102"}
          </p>
        </div>
      </div>
